    ...
    <div class="ruler " ref="testAA">
      <div class="main" :style="{'-webkit-transform': 'translateX(' +  mainValue + 'px)', 'width': getMainWidth+'px', position: 'relative',  left: '0px'}">
        <ul style="overflow: hidden;  -webkit-transform: translateX(45px);">
          <li v-touch:touchstart="onTouchStart" v-touch:touchmove="onTouchMove" v-touch:touchend="onTouchEnd" v-touch:touchcancel="onTouchEnd" v-for="(n,index) in itemCount" :key="n"><span class="num">{{min  + index * 5 }} </span>
          </li>
        </ul>
      </div>
      <div class="arrow"></div>
...
export default {
  name: 'DLRuler',
  props: ["value", "unit", "min", "max", "change"],
  components: {},
  data: function () {
    return {
      mainValue: 0,
  ...
  methods: {
    ...
    onTouchEnd (var1, var2) {
      this.change && this.change(this.numberValue);
